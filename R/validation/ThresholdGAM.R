...
#'                  modelID, median_cid. Produced by analysis/05_tree_accuracy.R.
...
ComputeImprovement <- function(cid_data, modelID,
  ...
  # Join grid parameters for smooth predictors
  grid_cols <- PARAM_GRID[, c("tree_length", "gain_loss", "n_char", "n_taxa")]
  grid_cols$gridTag <- GridTag(PARAM_GRID)
  merged <- merge(merged, grid_cols, by = "gridTag", all.x = TRUE)

  merged$chars_per_taxon <- merged$n_char / merged$n_taxa
  merged$rate_ratio      <- merged$gain_loss   # gain-to-loss ratio
...
FitThresholdGAM <- function(improvement_df, k = 10L, verbose = FALSE) {
  ...
  if (verbose) {
    ...
    # Silent k-index check — warn if any smooth is under-smoothed
    chk <- tryCatch(
...
ExtractThreshold <- function(gam_fit, predictor, data, n_grid = 500L) {
  ...
  # Build prediction frame: vary `predictor`, hold others at median
  ...
  # Find zero-crossing using uniroot on interpolated function
  # Look for sign change in pred_vals
  ...
  # Use the first zero-crossing (lowest parameter value)
  i   <- sign_changes[1]
  res <- tryCatch(
    uniroot(
      function(x) {
