...
for (ax in AXES) {
  ...
  save_fig(p, paste0("20_marginal_", ax, "_by_model"), width = 10, height = 6)
}
...
if ("part_rate" %in% names(sum_df) && any(!is.na(sum_df$part_rate))) {
  ...
  save_fig(p_pr, "21_marginal_part_rate_by_model_nt", width = 9, height = 6)
} else {
...
save_fig(p_win, "25_win_region_tree_length_gain_loss", width = 9, height = 6)

message("\n06_parameter_grid_by_model.R complete -- figures written to ", PATHS$fig_dir)
