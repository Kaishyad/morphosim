...
if (length(present_cols) >= 2L) {
  ...
  save_fig(p1, "16_rank_concordance_bump_chart", width = 10, height = 8)
} else {
...
if ("avg_mse_tree_len" %in% colnames(scorecard)) {
  ...
  save_fig(p3, "18_model_scatter_cid_vs_known_answer_mse", width = 10, height = 5.5)
} else {
...
if ("mse_tree_len" %in% colnames(cell_df)) {
  ...
  save_fig(p4, "19_gridcell_cid_vs_known_answer_mse_by_model", width = 13, height = 10)
} else {
  message("mse_tree_len not in cross-metric grid-cell table -- skipping plot 4.")
}

message("\n07_cross_metric_analysis.R complete -- figures written to ", PATHS$fig_dir)
