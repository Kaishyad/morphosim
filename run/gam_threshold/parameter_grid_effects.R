...
if (!is.null(thresh_df) && nrow(thresh_df) > 0L) {
  ...
  save_fig(p1, "12_threshold_crossing_vs_scenario_baseline", subdir = "gam_threshold", width = 10, height = 6)
  ...
  save_fig(p2, "13_threshold_heatmap_vs_scenario_baseline", subdir = "gam_threshold", width = 9, height = 7)
} else {
...
if (!is.null(cid_data)) {
  ...
  if (nrow(crossings) > 0L) {
    ...
    save_fig(p4, "15_threshold_crossing_vs_scenario_baseline", subdir = "gam_threshold", width = 10, height = 6)
  }
} else {
...
message("\nparameter_grid_effects.R complete -- figures written to ", PATHS$fig_dir)
