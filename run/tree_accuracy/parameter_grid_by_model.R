...
# marginal effect lines, one axis at a time, other axes averaged out
for (ax in AXES) {
  marg <- sum_df %>%
...
# part_rate is nt-only, so it gets its own figure rather than joining the loop above
...
# main-effect sensitivity summary: for each model x grid axis, correlation
# between the axis and cid
...
# win-region map: for the tree_length x gain_loss plane, which model wins
win_region <- sum_df %>%
