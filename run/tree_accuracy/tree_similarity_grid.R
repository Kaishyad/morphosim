...
if (nrow(mk_df) > 0L) {
  ...
  save_fig(p1, "26_tree_similarity_heatmap_mk", subdir = "tree_accuracy", width = 9, height = 12)
}
...
if (nrow(best_mk) > 0L) {
  p3 <- best_mk %>%
    ...
    theme(axis.text.x = element_blank(), axis.text.y = element_text(size = rel(0.55)),
         panel.grid = element_blank())
  save_fig(p3, "28_best_model_per_cell_mk", subdir = "tree_accuracy", width = 6, height = 12)
}
