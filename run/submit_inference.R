# Generate and submit SLURM jobs for all model × simulation combinations.

# TO DO:
#   Rscript analysis/submit_inference.R              # write SLURM scripts only
#   Rscript analysis/submit_inference.R --submit     # also run sbatch
#   Rscript analysis/submit_inference.R --requeue    # re-submit only failed runs
...
for (scenario in scenarios) {
  ...
  for (seed in seeds) {
    ...
    for (modelID in model_ids) {
      ...
      # Check all output files for both runs
      logs_done <- all(vapply(1:2, function(run) {
        f <- LogFile(scenario, GridTag(PARAM_GRID[1, ]), simID, modelID, run)
        # We check existence across all grid cells below; skip pre-check here
        FALSE
      }, logical(1)))

      # Per grid-cell check: skip if result .rds already exists
...
# --- Helper: write a filled SLURM script to disk ---
...
.WriteSlurmScript <- function(scenario, gridTag, repID, modelID, slurmPath) {
  ...
  # Extract seed integer from repID 
