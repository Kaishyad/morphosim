# checks mcmc convergence for every completed inference run
...
# lets --max-trees override the O(n^2) tree-distance cap from the command line
...
# existing results, so already-checked rows can be skipped
existing_df <- NULL
  ...
  for (gi in seq_len(nrow(grid))) {
    ...
    for (rep in seq_len(nRep)) {
      ...
      for (mid in model_ids) {
        logs_exist <- JobLogsComplete(scenario, gridTag, repID, mid, nRuns)
...
for (si in seq_along(SCENARIOS)) {
  ...
  for (ri in seq_len(nrow(completed))) {
    ...
    if (ri %% 50L == 0L) {
      ...
      # checkpoint progress so a slurm walltime kill or node failure doesn't lose everything checked so far
  ...
  # save this scenario's file with its own results merged in
...
# tree ess vs scalar ess comparison
...
# write requeue list (all failures)
if (SINGLE_MODEL_MODE) {
